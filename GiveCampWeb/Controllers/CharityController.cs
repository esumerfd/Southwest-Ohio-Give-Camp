...
namespace GiveCampWeb.Controllers
{
    public class CharityController : Controller
    {
        ...
        private void Validate(CharityRequirement charity, List<Technology> infrastructure, List<Technology> support)
        {
            ...
            {
                if (charity.PhoneNumber.Length > 15)
                    ModelState.AddModelError("PhoneNumber", "Phone Number cannot exceed 15 characters");
            }
            if (String.IsNullOrEmpty(charity.ContactName))
                ModelState.AddModelError("ContactName", "Contact Name is required");
            else
            {
                if (charity.ContactName.Length > 50)
                    ModelState.AddModelError("ContactName", "Contact Name cannot exceed 50 characters");
            }
            ...
            if (String.IsNullOrEmpty(charity.BackgroundInformation))
                ModelState.AddModelError("BackgroundInformation", "Charity Information is required");
            else
            {
            ...
            {
                if (charity.Email.Length > 256)
                    ModelState.AddModelError("Email", "Email cannot exceed 256 characters");
            }

            if(!String.IsNullOrEmpty(charity.CallInstruction) && charity.CallInstruction.Length > 1000)
               ModelState.AddModelError("CallInstruction", "Best Time to Contact Information cannot exceed 1000 characters");

            if (!String.IsNullOrEmpty(charity.OtherInfrastructure) && charity.OtherInfrastructure.Length > 1000)
                ModelState.AddModelError("OtherInfrastructure", "Additional Infrastructure cannot exceed 1000 characters");

            if (!string.IsNullOrEmpty(charity.OtherSupportSkills) && charity.OtherSupportSkills.Length > 1000)
